"""Author's hypothesis (2026-09-19 00:40): the fringe mismatch of the swapped-sigma P2-04 fit comes from a light surface
...
def request(cap, sC, sCo):
    ...
    return {"measurement_id": "P2-04/xrr.dat", "lambda": 1.5406, "polarization": "s",
            ...
            "points_inline_max": 0, "seed": 20260918}

CASES = [("A-S nocap, start swapped", False, 2.6, 4.3), ("A-R nocap, start author", False, 5.6, 2.5),
         ("B-S cap, start swapped", True, 2.6, 4.3), ("B-R cap, start author", True, 5.6, 2.5)]
results = {}

def run(name, cap, sC, sCo):
    w = os.path.join(ROOT, name.split()[0])
    p, call = server(w)
    try:
        j = call("fit_xrr", request(cap, sC, sCo))["job_id"]
        while True:
