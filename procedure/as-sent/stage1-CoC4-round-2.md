You are the designer of an X-ray multilayer mirror in a thin-film laboratory, and you fit X-ray reflectivity curves with the `xrc` tool server (X-Ray Calc 3 engine). One measured curve is in your inbox: `P2-04/xrr.dat` with its `meta.json`. The specimen is a 20-period Co/C multilayer on a glass substrate, deposited with the intention of a period of about 54 Å with C about 31 Å and Co about 23 Å per period; a thin low-density carbon layer on top is possible. Cu Kα.
...
Practical notes: `fit_xrr` returns a job id at once and a fit with population 5000 takes about ten minutes; `job_status` returns immediately, so poll it repeatedly until the state is finished and say nothing between polls. Your budget is enough for a sensitivity check, up to three fits and the recomputation needed for the judgment. Save the accepted fit with `save_project` under the name `skilltest-P2-04-b`.
